...
def remove_dots_leaving_screen(dotsX, dotsY, deathDots):
    ...
    outfieldDotsX = numpy.logical_or((dotsX >= screenSizeX), (dotsX <= -screenSizeX))
    outfieldDotsY = numpy.logical_or((dotsY >= screenSizeY), (dotsY <= -screenSizeY))

    outfieldDots = numpy.logical_or(outfieldDotsX, outfieldDotsY)
    randDotsX[outfieldDotsX] = numpy.random.uniform(low=-screenSizeX, high=screenSizeX,size=(sum(outfieldDotsX,)))
    randDotsY[outfieldDotsY] = numpy.random.uniform(low=-screenSizeY, high=screenSizeY,size=(sum(outfieldDotsY,)))
...
def randomDotMove(randDotsX, randDotsY, randDots, veloX, veloY, deathDots, cookGroup, moveDir, hori, targetLoc, groupElementShape, targetShape, groupingElementsSpeed, aperture_xy, targetExists, diffGroupingDirections, sharedMotion, targetSpeed):
    ...
    outfieldDotsY = numpy.logical_or((randDotsY >= screenSizeY), (randDotsY <= -screenSizeY))
...
#randomFrameList_trial = numpy.load('p7_trial_list.npy')

numpy.save(fileName, randomFrameList_trial) #45degrees_350_Gspeed3
